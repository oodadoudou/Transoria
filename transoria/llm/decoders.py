...
def decode_glossary_jsonl(raw: str) -> GlossaryDecodeResult:
    ...
    Off-spec responses (Markdown tables, prose, etc.) yield zero entries
    rather than corrupted data. Earlier versions tried to salvage rows
    from Markdown tables, but the LLM's column order is unstable and the
    salvage produced misaligned entries (src=row-number, dst=category).
    Failure must be visible upstream so a retry can fix the prompt.
    """
