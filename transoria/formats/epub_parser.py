    ...
    *,
    ...
    segments.append(
        EpubTextSegment(
            ...
            doc_path=doc_path,
            block_path=block_path,
            text=join_segment_text(part_texts, parts, raw_texts),
...
def join_segment_text(
    part_texts: list[str], parts: list[EpubPartRef], raw_texts: list[str] | None = None
) -> str:
    if any("/ruby[" in part.path for part in parts):
        return "".join(part_texts)
    if not raw_texts:
        return "\n".join(part_texts)
    fragments = [part_texts[0]]
    for index in range(1, len(parts)):
        if "/$node[" in parts[index].path or "/$node[" in parts[index - 1].path:
            left = raw_texts[index - 1]
            right = raw_texts[index]
            separator = " " if (left and left[-1].isspace()) or (right and right[0].isspace()) else ""
        else:
            separator = "\n"
        fragments.extend((separator, part_texts[index]))
    ...
    for child in elem:
        ...
        if collect_direct_slots and child.tail:
            units.append(
                (
                    elem_path,
                    [(EpubPartRef(slot="tail", path=_text_slot_path(child, path_map)), child.tail)],
                )
            )
    ...
    def walk(elem: etree._Element) -> None:
        ...
        for child in elem:
            if isinstance(child.tag, str):
                walk(child)
            if child.tail:
                results.append((EpubPartRef(slot="tail", path=_text_slot_path(child, path_map)), child.tail))
...
def find_by_path(root: etree._Element, path: str) -> etree._Element | None:
    ...
    for part in parts[1:]:
        ...
        if name == "$node":
            matches = [child for child in current if not isinstance(child.tag, str)]
        else:
...
def _text_slot_path(
    node: etree._Element, path_map: dict[etree._Element, str]
) -> str:
    if isinstance(node.tag, str):
        return path_map[node]
    parent = node.getparent()
    if parent is None:
        raise ValueError("Text slot node has no parent")
    position = 0
    for child in parent:
        if isinstance(child.tag, str):
            continue
        position += 1
        if child is node:
            return f"{path_map[parent]}/$node[{position}]"
    raise ValueError("Text slot node is detached from its parent")
