...
def test_format_context_section_uses_negative_indices_for_prior_lines() -> None:
    section = format_context_section(("first", "second", "third"))

    parsed = [json.loads(line) for line in section.splitlines()]
    assert parsed == [{"-3": "first"}, {"-2": "second"}, {"-1": "third"}]
