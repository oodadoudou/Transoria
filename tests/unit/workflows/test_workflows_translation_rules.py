...
def test_glossary_match_many_keeps_semantic_entry_variants() -> None:
    glossary = Glossary(
        entries=(
            GlossaryEntry(src="John", dst="约翰", case_sensitive=True),
            GlossaryEntry(src="John", dst="约翰", case_sensitive=False),
            GlossaryEntry(src=r"John", dst="约翰", regex=True),
        )
    )

    matches = glossary.match_many(["John said hello", "John waved"])

    assert len(matches) == 3
    assert {
        (entry.regex, entry.case_sensitive) for entry in matches
    } == {
        (False, True),
        (False, False),
        (True, False),
