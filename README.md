...
- **质量检查与校对**：按源语言识别低置信度、原文残留、术语异常、疑似重复和模型异常；支持单条、批量及筛选结果重译，并对韩语源任务的单条重译及疑似外语音译还原进行候选质量校验；拉丁语系批量重译会拦截明显的段落错配。
